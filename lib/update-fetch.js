...
const UNBRICK_TGZ_URL = 'https://s3.amazonaws.com/builds.tessel.io/custom/new_build_next.tar.gz'
const UNBRICK_UBOOT_FILE = 'openwrt-ramips-mt7620-Default-u-boot.bin'
const UNBRICK_SQASHFS_FILE = 'openwrt-ramips-mt7620-tessel-squashfs-sysupgrade.bin'
...
function fetchUnbrickFiles() {
  var fileMap = {
    uboot: UNBRICK_UBOOT_FILE,
    squashfs: UNBRICK_SQASHFS_FILE
  };
  return downloadTgz(UNBRICK_TGZ_URL, fileMap);
}
...
function downloadTgz(tgzUrl, fileMap){
  return new Promise(function(resolve, reject) {
    logs.info('Beginning download. This could take a couple minutes..');
    ...
    extract.once('finish', function() {
      for (var key in files){
        var file = files[key];
        if (!file.length) return reject(new Error('Fetched file wasn\'t formatted properly.'));
      }
...
module.exports.fetchUnbrick = fetchUnbrickFiles;
