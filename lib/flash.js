...
function factoryPartition (mac1, mac2) {
  var command = [0x20, 0x76, 0x03, 0x01]
    .concat(mac1)
    .concat(Array(30).fill(0xff))
    .concat(mac2);
...
function transaction (usbConnection, write, read, status_poll, wren, next) {
  ...
  var flags = parseInt(status_poll) | (parseInt(wren) << 1)
  var hdr = [(read >> 0) & 0xff, (read >> 8) & 0xff, (read >> 16) & 0xff, flags]

  var data = hdr.concat(write)

  usbConnection.epOut.transfer(data, function (err) {
    if (err) return next(err)
    if (read > 0) return usbConnection.epIn.transfer(read, next)
    next()
  })
}
...
function write(usbConnection, writeAddress, data, sliceStart, next){
  ...
  if (sliceStart >= data.length) return next()
  ...
  writePage(usbConnection, writeAddress, pageData, function onWrite(err){
    if (err) return next(err)
    write(usbConnection, writeAddress + PAGE, data, sliceEnd, next)
  })
}
...
function writePage (usbConnection, addr, data, next) {
  ...
  transaction (
    usbConnection,
    [0x12].concat(address(addr)).concat(data),
    null,
    status_poll,
    wren,
    next
...
function resetOpenwrt (device, next) {

  async.series([

    asyncLog.bind(null, 'Reset the USB interface...'),
    device.reset.bind(device),

    asyncLog.bind(null, 'Control transfer to put RST line high...'),
    device.controlTransfer.bind(device, 0x40, 0x10, 1, 0, ''),

    asyncLog.bind(null, 'Control transfer to put RST line low...'),
    device.controlTransfer.bind(device, 0x40, 0x10, 0, 0, ''),
...
module.exports = function flashDevice (usbConnection, ubootBuffer, squashfsBuffer, next) {
  ...
  async.waterfall([
    ...
    asyncLog.bind(null, "Rebooting device..."),
    resetOpenwrt.bind(null, usbConnection.intf.device)
