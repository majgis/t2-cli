...
function resetOpenwrt (usbConnection, next) {

  usbConnection.open(2).then(function onSetAltSetting(){
    var device = usbConnection.device
    async.series([
...
module.exports = function flashDevice (usbConnection, ubootBuffer, squashfsBuffer, next) {
  ...
  async.waterfall([
    ...
    resetOpenwrt.bind(null, usbConnection)
