...
Tessel.get = function(opts) {
  return new Promise(function(resolve, reject) {
    ...
    function searchComplete() {
      if (opts.connectionsOnly){
        if (connections.length ===0){
          return reject('No Tessel connections were found.')
        } else {
    ...
    seeker.on('connection', function(connection){
...
var OPENWRT_UNBRICK_URL = 'https://s3.amazonaws.com/builds.tessel.io/custom/new_build_next.zip'
controller.unbrickTessel = function(opts){
  opts.usb = true;
  opts.connectionsOnly = true;
  return new Promise(function(resolve, reject){
    controller.get(opts).then(function(connections) {
      var connectionCount = connections.length;

      if (connectionCount === 0) {
        return reject(new Error('No connections were found.'));
      }

      if (connectionCount > 1){
        return reject(new Error(
          `There were tessel ${connectionCount} connections found. \
Disconnect all but the tessel you wish to restore and try again.`
        ));
      }

      logs.info('One tessel connection was found.');

      unbrick(opts, connections[0])
